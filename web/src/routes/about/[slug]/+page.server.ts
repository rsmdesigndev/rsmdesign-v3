...
export const _query = gql`
	query AboutSubpages($slug: String!) {
		about_subpages(filter: { slug: { _eq: $slug } }) {
			slug
			seo_page_title
			seo_page_description
			about_subpage_page_blocks_v3 {
				item {
					__typename
...
export const load: PageServerLoad = async ({ params }) => {
	const res = await cmsClient.AboutSubpages({ slug: params.slug });

	if (res.about_subpages.length === 0) {
		throw error(404);
	}

	return {
		subpage: {
			...res.about_subpages[0],
			about_subpage_page_blocks_v3: await prefetchDataFeeds(res.about_subpages[0]?.about_subpage_page_blocks_v3)
		}
	};
};
