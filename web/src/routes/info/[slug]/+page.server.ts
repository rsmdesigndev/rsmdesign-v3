...
export const _query = gql`
	query InfoPages($slug: String!) {
		info(filter: { slug: { _eq: $slug } }) {
			slug
			seo_page_title
...
export const load: PageServerLoad = async ({ params }) => {
	const res = await cmsClient.InfoPages({ slug: params.slug });
	...
	return {
		page: {
			...res.info[0],
			info_page_blocks_v3: await prefetchDataFeeds(res.info[0]?.info_page_blocks_v3)
		}
	};
};
