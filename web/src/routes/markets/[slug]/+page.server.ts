...
export const _query = gql`
	query Market($slug: String!) {
		markets(filter: { slug: { _eq: $slug }, visibility: { _neq: "archived" } }) {
			date_created
			date_updated
			...
			markets_use_page_blocks_v3
			markets_page_blocks_v3 {
				item {
					__typename
...
export const load: PageServerLoad = async ({ params }) => {
	const marketsRes = await cmsClient.Market({ slug: params.slug });
	if (marketsRes.markets.length === 0) {
		throw error(404);
	}
	const market = {
		...marketsRes.markets[0],
		markets_page_blocks_v3: await prefetchDataFeeds(marketsRes.markets[0]?.markets_page_blocks_v3)
	};

	const featuredProjects = market.featured_projects?.map((p) => p?.projects_id ?? {}) ?? [];
