...
export const _query = gql`
	query AeoSlug($slug: String!) {
		aeo_articles(filter: { slug: { _eq: $slug }, aeo_article_visibility: { _neq: "feed" } }) {
			slug
			aeo_article_visibility
			aeo_article_heading
			aeo_article_page_description
			aeo_article_use_page_blocks_v3
			aeo_article_page_blocks_v3 {
				item {
					__typename
...
export const load: PageServerLoad = async ({ params }) => {
	...
	return {
		article: {
			...res.aeo_articles[0],
			aeo_article_page_blocks_v3: await prefetchDataFeeds(res.aeo_articles[0]?.aeo_article_page_blocks_v3)
		}
	};
};
