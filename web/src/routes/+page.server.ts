...
export const _query = gql`
	query Homepage {
		...
		projects_in_progress: projects(
			limit: 3,
			sort: ["sort", "project_opening_date"],
			filter: { services: { services_id: { slug: { _eq: "in-progress" } } }, visibility: { _in: ["hiddenOnWork", "visible"] } }
		) {
			slug
			project_title
			project_title_short
		...
		slider_projects: projects(filter: { home_page_slider: { _eq: true }, visibility: { _nin: ["hidden", "archived"] } }
		) {
			slug
			project_title
			location
...
export const load: PageServerLoad = async ({ params }) => {
	const res = await cmsClient.Homepage();

	const projectsLimit = 9;
	const projectsFilter = { visibility: { _nin: ["hidden", "archived"] } };
	...
	return {
		...
		projectsInProgress: res.projects_in_progress,
		sliderProjects: res.slider_projects,
