...
<template>
	<header>
		<div class="wrapper">
			<div class="container flex space-between align-center">
				<div class="logo" class:active={menuOpen}>
					<a href="/" aria-label="Navigate to homepage" on:click={closeMenu}>
	...
	<div class="bg" style:--HEADER-HEIGHT={headerHeight} />
</template>
...
<style lang="scss">
	header {
		...
		> div.scrim {
			...
			width: 100%;
			height: 166.667%;
			//background: var(--bg-scrim, linear-gradient(rgba(255,255,255,0.2),rgba(255,255,255,0.133),rgba(255,255,255,0)));
		}
	}
	...
	.menu-button {
		...
		--menu-button-width: calc(1rem * 25 / 16);
		...
		> div {
			...
			width: 100%;
			height: calc(var(--menu-button-width) / 10);
			margin: calc(var(--menu-button-width) / 6) 0;
		...
		&.active {
			--menu-button-transform: calc(var(--menu-button-width) * 3 / 16);
			//transform: translateX(var(--menu-button-transform));

			> div {
				background-color: white;
				width: calc(100% * sqrt(2) / 2);
