<script lang="ts">
  ...
  // Types
  interface PageBlockAdvancedProjectGridData {
    heading?: string | null;
  ...
  // Props
  export let data = <PageBlockAdvancedProjectGridData>{};
...
<Section order={order}>
  <div class="advanced-project-grid">
    {#if data.heading}
      <h2 class="advanced-project-grid__heading {data.heading_size}">{data.heading}</h2>
    {/if}
